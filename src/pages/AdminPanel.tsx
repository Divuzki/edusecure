...
const AdminPanel: React.FC = () => {
  ...
  const fetchStorageStats = async () => {
    try {
      ...
      const totalSize =
        files?.reduce((sum, file) => sum + (file.size || 0), 0) || 0;
  ...
  const deleteUser = async (userId: string) => {
    if (
      !window.confirm(
        "Are you sure you want to delete this user? This will also delete all their files."
      )
    ) {
      return;
    }
    ...
    try {
      ...
      await supabase.from("share_links").delete().eq("owner_id", userId);
          ...
          <div className="p-6">
            {Object.keys(storageStats.filesByType).length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {Object.entries(storageStats.filesByType).map(
                  ([type, count]) => (
                    <div key={type} className="bg-gray-50 rounded-lg p-4">
                      ...
                      <div className="mt-2 w-full bg-gray-200 rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${
                              (count / storageStats.totalFiles) * 100
                            }%`,
                          }}
                        ></div>
                      </div>
