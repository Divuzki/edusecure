...
const CloudStorageSetup: React.FC = () => {
  ...
  const onSubmit = async (data: CloudStorageFormInputs) => {
    ...
    try {
      ...
      setTestResult({
        success: true,
        message: `Successfully connected to ${data.name} (${data.provider.toUpperCase()})`,
      });
      ...
      setTestResult({
        success: false,
        message: 'Failed to connect. Please check your credentials and try again.',
      });
    } finally {
  ...
  return (
    <Box>
      <Heading size="lg" mb={6}>Cloud Storage Setup</Heading>
      ...
      >
        ...
        <Text mb={6} color={useColorModeValue('gray.600', 'gray.400')}>
          Connect your cloud storage provider to securely store and manage educational data.
          ...
          <Stack spacing={4}>
            <FormControl id="provider" isInvalid={!!errors.provider}>
              ...
              >
                ...
                <option value="gcp">Google Cloud Platform (Cloud Storage)</option>
              </Select>
            ...
            {selectedProvider === 'aws' && (
              <>
                ...
                <FormControl id="accessKeyId" isInvalid={!!errors.accessKeyId}>
                  ...
                  />
                  <FormErrorMessage>{errors.accessKeyId?.message}</FormErrorMessage>
                </FormControl>
                
                <FormControl id="secretAccessKey" isInvalid={!!errors.secretAccessKey}>
                  ...
                  <InputGroup>
                    ...
                    <InputRightElement>
                      <IconButton
                        ...
                        icon={showSecret ? <EyeOff size={18} /> : <Eye size={18} />}
                        variant="ghost"
                        size="sm"
                  ...
                  <FormErrorMessage>{errors.secretAccessKey?.message}</FormErrorMessage>
                </FormControl>
              </>
            )}
            
            {selectedProvider === 'azure' && (
              <FormControl id="connectionString" isInvalid={!!errors.connectionString}>
                ...
                <FormErrorMessage>{errors.connectionString?.message}</FormErrorMessage>
              </FormControl>
            )}
            ...
            {selectedProvider === 'gcp' && (
              <>
                ...
                <FormControl id="keyFilename" isInvalid={!!errors.keyFilename}>
                  ...
                  <FormErrorMessage>{errors.keyFilename?.message}</FormErrorMessage>
                </FormControl>
              </>
            )}
