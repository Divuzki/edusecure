...
const EssaySubmission: React.FC = () => {
    ...
    const loadModel = async () => {
      try {
        ...
        toast({
          ...
          description: 'Failed to load the essay scoring model. Some features may be limited.',
          status: 'error',
          duration: 5000,
          isClosable: true,
        });
  ...
  const handleScoreEssay = async () => {
    if (!essayContent || essayContent.trim().length < 100) {
      toast({
        ...
        description: 'Please write at least 100 characters for accurate scoring.',
        status: 'warning',
        duration: 3000,
        isClosable: true,
      });
  ...
  const onSubmit = async (data: EssaySubmissionFormInputs) => {
    ...
    try {
      ...
      const file = new File([data.content], `${data.title.replace(/\s+/g, '_')}.txt`, {
        type: 'text/plain',
      });
      ...
      toast({
        ...
        description: 'Your essay has been encrypted and submitted successfully.',
        status: 'success',
        duration: 5000,
        isClosable: true,
      });
  ...
  return (
    <Box>
      <Heading size="lg" mb={6}>Essay Submission</Heading>
          ...
          <Stack spacing={4}>
            ...
            <FormControl id="content" isInvalid={!!errors.content}>
              ...
              <Text fontSize="sm" color="gray.500" mt={1}>
                {essayContent ? `${essayContent.length} characters` : '0 characters'}
              </Text>
            </FormControl>
            ...
            <Flex justify="space-between" mt={4}>
              <Button
                ...
                isLoading={isScoring}
                loadingText="Scoring"
                isDisabled={modelLoading || !essayContent || essayContent.length < 100}
              >
                Score Essay
              </Button>
